...
export default function EditTeacherModal({ teacher, onClose, onSuccess }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    phone: teacher.phone || '',
    bio: teacher.bio || '',
    join_date: teacher.join_date ? new Date(teacher.join_date).toISOString().slice(0, 10) : '',
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(teacher.photo_url || null)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.put(`/api/pms/teachers/${teacher.id}`, payload)

      if (imageFile) {
        const fd = new FormData()
        fd.append('file', imageFile)
        await api.post(`/api/pms/teachers/${teacher.id}/photo`, fd)
      }

      onSuccess()
    } catch (e: any) {
      setError(e.response?.data?.detail || e.message || 'Error al actualizar profesor')
    } finally {
      setLoading(false)
    }
  }
           ...
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              ...
              <div className="lg:col-span-4 space-y-6">
                 ...
                 <div className="p-6 bg-gray-50 rounded-3xl border border-gray-100">
                    <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Resumen de Cuenta</div>
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-xs font-bold">
                        ...
                        <span className="text-gray-600">{teacher.join_date ? new Date(teacher.join_date).toLocaleDateString() : '-'}</span>
                      </div>
                    </div>
                 </div>
              </div>
              ...
              <div className="lg:col-span-8 space-y-8">
                 ...
                 <div className="space-y-6">
                    <div className="flex items-center gap-2 px-2">
                       ...
                       <h3 className="text-xs font-black text-gray-900 uppercase tracking-widest">Información Personal</h3>
                    </div>
                 ...
                 <div className="space-y-6">
                    <div className="flex items-center gap-2 px-2">
                       ...
                       <h3 className="text-xs font-black text-gray-900 uppercase tracking-widest">Historial Profesional</h3>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       <div className="space-y-2">
                          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-2">Fecha ingreso</label>
                          <div className="relative group">
                       ...
                       <div className="space-y-2">
                          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-2">Fecha nacimiento</label>
                          <div className="relative group">
                             <HiOutlineCalendar className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-fuchsia-500 transition-colors" size={20} />
                             <input 
                                type="date"
                       ...
                       <div className="space-y-2 md:col-span-2">
                          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-2">Estilos / Especialidades</label>
                          <div className="relative group">
                       ...
                       <div className="space-y-2 md:col-span-2">
                          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-2">Bio / Perfil Profesional</label>
                          <div className="relative group">
